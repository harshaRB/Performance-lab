...
import {
    Moon, Apple, Dumbbell, Brain, Smartphone, Droplets,
 ...
 * 
 * Now responsive - scales based on container width.
 */

// Module configuration (now includes HYDRATION!)
const MODULES = [
    { key: 'sleep', label: 'Sleep', icon: Moon, targetScore: 80, weight: 0.25 },
    { key: 'nutrition', label: 'Nutrition', icon: Apple, targetScore: 75, weight: 0.20 },
    { key: 'training', label: 'Training', icon: Dumbbell, targetScore: 70, weight: 0.15 },
    ...
    { key: 'hydration', label: 'Hydration', icon: Droplets, targetScore: 70, weight: 0.10 }, // NEW!
];
...
const SystemScoreHub = ({
    scores = {},
    size: propSize = 400,
    responsive = true, // Enable responsive mode by default
}) => {
    const containerRef = useRef(null);
    const [containerWidth, setContainerWidth] = useState(propSize);
    ...
    useEffect(() => {
        ...
        const updateSize = () => {
            if (containerRef.current) {
                ...
                // Clamp between 280 (mobile min) and 400 (desktop max)
                setContainerWidth(Math.min(400, Math.max(280, width)));
            }
        };
    ...
    const scaledNodeRadius = Math.max(20, nodeRadius * scaleFactor);
    const scaledFontSize = Math.max(24, 32 * scaleFactor);
        ...
        <div ref={containerRef} style={{ width: '100%', maxWidth: '400px', margin: '0 auto' }}>
            <motion.div
                style={{
                    position: 'relative',
                    width: size,
                    height: size + (scaleFactor < 0.8 ? 80 : 120), // Less space on mobile
                    margin: '0 auto',
                }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
